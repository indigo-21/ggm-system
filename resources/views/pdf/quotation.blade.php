...
@section("content")
    ...
    <div class="divider"></div>
        ...
        <table class="table-bordered">
            <tr>
                <td colspan="4" style="text-align:center; font-weight:bold;">{{ $headStone }} - {{ $headStoneSize }} - {{ $material }}</td>
            </tr>
            @if ($orderCost->description && $orderCost->amount)
                <tr>    
                    <td class="no-border" colspan="2">{{ $orderCost->description }}</td>
                    <td class="no-border" colspan="2" style="text-align:right"> £ {{$orderCost->amount ? number_format($orderCost->amount, 2) : "0" }}</td>
                
                </tr>
            @endif
            @if ($orderCost->letter_count && $orderCost->letter_amount)
                <tr>
                    <td class="no-border" colspan="2">{{$orderCost->letter_count}} Letters @ £ {{ number_format($orderCost->letter_amount, 2) }}</td>
                    <td class="no-border" colspan="2" style="text-align:right;">£ {{$orderCost->letter_total_amount}}</td>
                </tr>
            @endif
            ...
            @foreach ($orderCostAdditionals as $additional )
                <tr>
                    ...
                    <td class="no-border" colspan="2" style="text-align:right">£ {{number_format($additional->amount, 2)}}</td>
                </tr>
            @endforeach
        </table>
        <br>
