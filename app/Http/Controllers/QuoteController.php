    ...
    public function default_required_data($isFrom = 'index', $id = false)
    {
        ...
        if ($isFrom == 'form') {
            $data += [
                ...
                // The form is shared by create and edit.  Always provide the
                // optional relationship keys so its null-safe expressions have
                // a defined variable to operate on during quote creation.
                'order_cost' => null,
                'order_note' => null,
                'order_payments' => collect(),
                'total_deposit' => 0,
                'order_balance' => 0,
            ];
            if ($id) {
